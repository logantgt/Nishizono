...
{
    public static MemoryStream PlotImmersionLogs(List<ImmersionLog> logs)
    {
        ...
        myPlot.Grid.MajorLineColor = Colors.Gray; // subtle grid lines
        ...
        for (int x = 0; x < uniqueDayCount; x++)
        {
            double vn = 0, manga = 0, anime = 0, book = 0, listening = 0, youtube = 0, anki = 0;

            foreach (ImmersionLog log in logs.Where(_ => _.TimeStamp.Date == oldest.Date.AddDays(x)))
            {
        ...
        for (int x = 0; x < uniqueDayCount; x++)
        {
            tickGen.AddMajor(x, oldest.AddDays(x).Date.ToShortDateString());
        }
        ...
        myPlot.Legend.OutlineColor = Colors.Gray;
